...
TEST_CASE("Table of coefficients", "[table]"){
  ...
  lookupTable.setCoefficientsAtPoint(Utils::Point(2.214, 0.123), ref2);
  coeffs = lookupTable.getCoefficientsAtPoint(p);
  REQUIRE(!lookupTable.isEmptyAtPoint(Utils::Point(2.213, 0.123)));
  REQUIRE(coeffs == ref2);
  ...
  REQUIRE(!lookupTable.isEmptyAtPoint(Utils::Point(2.0, 0.0)));
  REQUIRE(!lookupTable.isEmptyAtPoint(Utils::Point(2.999, 0.0)));
  REQUIRE(!lookupTable.isEmptyAtPoint(Utils::Point(2.0, 0.999)));
  REQUIRE(!lookupTable.isEmptyAtPoint(Utils::Point(2.999, 0.999)));
  REQUIRE(!lookupTable.isEmptyAtPoint(Utils::Point(2.001, 0.99)));
  REQUIRE(!lookupTable.isEmptyAtPoint(Utils::Point(2.731, 0.0001)));
  REQUIRE(lookupTable.isEmptyAtPoint(Utils::Point(3.731, 0.0001)));
